...
const FuncionalidadCarrito = () => {
    ...
    useEffect(() => {
        const fetchCarrito = async () => {
            try {
                ...
                const response = await fetch(`https://wisteria-2cn8.onrender.com/item-ordenes/carrito/${userId}`, {
                    method: 'GET',
                    headers: {
    ...
    const actualizarCantidad = async (idItemOrden: number, nuevaCantidad: number,idProducto: number) => {
        const token = localStorage.getItem('token') || '';
        const response = await fetch(`https://wisteria-2cn8.onrender.com/item-ordenes/${idItemOrden}`, {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
    ...
    const eliminar = async (idItemOrden: number) => {
        ...
        const response = await fetch(`https://wisteria-2cn8.onrender.com/item-ordenes/orden/${idItemOrden}`, {
            method: 'DELETE',
            headers: {
