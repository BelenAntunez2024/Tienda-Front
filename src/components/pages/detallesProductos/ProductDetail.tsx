...
const ProductDetail: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        setLoading(true);
        const token = localStorage.getItem('token') || '';
        const response = await fetch(`https://wisteria-2cn8.onrender.com/producto/${id_producto}`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
  ...
  if (yaEnCarrito) {
    ...
    try {
      ...
      const response = await fetch("https://wisteria-2cn8.onrender.com/item-ordenes", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          'Authorization': `Bearer ${token}`
